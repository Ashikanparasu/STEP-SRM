...
public class TextCompressor {
    ...
    public static int[] frequencies(String text,char[] chars) {
        ...
        for(int i=0;i<text.length();i++) {
            char c=text.charAt(i);
            for(int j=0;j<chars.length;j++) if(chars[j]==c) freq[j]++;
        }
        return freq;
    }
    public static String[] codes(char[] chars,int[] freq) {
        String[] codes=new String[chars.length];
        for(int i=0;i<chars.length;i++) codes[i]=Integer.toString(i,36);
        return codes;
    }
    public static String compress(String text,char[] chars,String[] codes) {
        ...
        for(int i=0;i<text.length();i++) {
            char c=text.charAt(i);
            for(int j=0;j<chars.length;j++) if(chars[j]==c) sb.append(codes[j]);
        }
        return sb.toString();
    }
